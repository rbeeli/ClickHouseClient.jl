...
function read_col_data(sock::ClickHouseSock, num_rows::VarUInt,
    ...
    for i in eachindex(missing_map)
        marker = missing_map[i]
        if marker == 0x01
            result[i] = missing
        elseif marker != 0x00
            throw(ArgumentError("Nullable null map contains invalid byte 0x$(string(marker, base=16))"))
        end
    end
    return result
end
...
missing_replacement(::Type{DateTime64{P}}) where {P} = DateTime64{P}(0)
missing_replacement(::Type{TimeZones.ZonedDateTime}) =
    TimeZones.ZonedDateTime(unix2datetime(0), CLICKHOUSE_UTC_TIMEZONE)
missing_replacement(::Type{ClickHouseTime}) = ClickHouseTime(0)
missing_replacement(::Type{ClickHouseTime64{P}}) where {P} = ClickHouseTime64{P}(0)
missing_replacement(::Type{ClickHouseZonedDateTime64{P}}) where {P} =
    ClickHouseZonedDateTime64{P}(0, CLICKHOUSE_UTC_TIMEZONE)
missing_replacement(::Type{ClickHouseDecimal32{S}}) where {S} = ClickHouseDecimal32{S}(0)
missing_replacement(::Type{ClickHouseDecimal64{S}}) where {S} = ClickHouseDecimal64{S}(0)
missing_replacement(::Type{ClickHouseDecimal128{S}}) where {S} = ClickHouseDecimal128{S}(0)
missing_replacement(::Type{ClickHouseDecimal256{S}}) where {S} = ClickHouseDecimal256{S}(0)
